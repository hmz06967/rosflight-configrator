...
TABS.configuration.initialize = function (callback, scrollPosition) {
    ...
    function load_estconf(){

            $('input[name="PARAM_INIT_TIME"]').val(rosflightIO.handle_get_param("FILTER_INIT_T").toFixed(2))
            $('input[name="PARAM_FILTER_KP_ACC"]').val(rosflightIO.handle_get_param("FILTER_KP_ACC").toFixed(4) ).attr("step", 0.001);
            $('input[name="PARAM_FILTER_KI"]').val(rosflightIO.handle_get_param("FILTER_KI").toFixed(10)  * 100).attr("step", 0.00001);
            $('input[name="PARAM_FILTER_KP_EXT"]').val(rosflightIO.handle_get_param("FILTER_KP_EXT").toFixed(4));
            $('input[name="PARAM_FILTER_ACCEL_MARGIN"]').val(rosflightIO.handle_get_param("FILTER_ACCMARGIN").toFixed(2) *100) .attr("step", 0.01);
            $('input[name="PARAM_GYRO_XY_ALPHA"]').val(rosflightIO.handle_get_param("GYROXY_LPF_ALPHA").toFixed(2) *100) .attr("step", 0.01);
            $('input[name="PARAM_GYRO_Z_ALPHA"]').val(rosflightIO.handle_get_param("GYROZ_LPF_ALPHA").toFixed(2)*100  ) .attr("step", 0.01);
            $('input[name="PARAM_ACC_ALPHA"]').val(rosflightIO.handle_get_param("ACC_LPF_ALPHA").toFixed(2)*100 ) .attr("step", 0.01);
            $('input[name="PARAM_GROUND_LEVEL"]').val(rosflightIO.handle_get_param("GROUND_LEVEL").toFixed(2) ) ;
    }
    
    function load_config() {
        //MSP.send_message(MSPCodes.MSP_FEATURE_CONFIG, false, false, load_beeper_config);
        
        if(rosflightIO.parameter_list.length > 112){
            
            /*MAV.send_message(mavlink10.MAVLINK_MSG_ID_PARAM_SET, {param_id: 'MIXER', param_value: 1, param_type: 6}, 0, ()=>{
           
            }, 1);*/

            MIXER_CONFIG.mixer = rosflightIO.handle_get_param("MIXER");  
            MIXER_CONFIG.mixer = MIXER_CONFIG.mixer > 14 ? 0 : MIXER_CONFIG.mixer;
            MIXER_CONFIG.reverseMotorDir = rosflightIO.handle_get_param("ELEVATOR_REV");   
            
            BOARD_ALIGNMENT_CONFIG.roll = rosflightIO.parameter_list[104].param_value;  
            BOARD_ALIGNMENT_CONFIG.pitch = rosflightIO.parameter_list[105].param_value;  
            BOARD_ALIGNMENT_CONFIG.yaw = rosflightIO.parameter_list[106].param_value;  
            
            MOTOR_CONFIG.minthrottle = 0;
            MOTOR_CONFIG.maxthrottle = 2000;
            $('input[name="minthrottle"], input[name="maxthrottle"]').attr("min", MOTOR_CONFIG.minthrottle.toString());
            $('input[name="maxthrottle"], input[name="minthrottle"]').attr("min", MOTOR_CONFIG.maxthrottle.toString());
            //$('input[name="minthrottle"], input[name="maxthrottle"]').attr("step","0.1");
            MOTOR_CONFIG.mincommand = rosflightIO.handle_get_param("MOTOR_IDLE_THR") * 2000;  
    ...
    function load_estimator_config(){
    ...
    function process_html() {
        ...
        function refreshMixerPreview() {
            var mixer = (!isNaN(MIXER_CONFIG.mixer) && MIXER_CONFIG.mixer > mixerList.length) ? 0 : MIXER_CONFIG.mixer; 
            var reverse = MIXER_CONFIG.reverseMotorDir ? "_reversed" : "";
            if(mixerList[mixer]!=undefined)$('.mixerPreview img').attr('src', './resources/motor_order/' + mixerList[mixer].image + reverse + '.svg');
        };
        ...
        $('a.save').click(function () {
            ...
            function save_misc() {
                ...
                if(false) {
                    //MSP.send_message(MSPCodes.MSP_SET_MISC, mspHelper.crunch(MSPCodes.MSP_SET_MISC), false, next_callback);
                                        ...
                                        var param1 = function (){
                        var ins = $('input[name="PARAM_FILTER_KP_ACC"]');
                        var scale = parseFloat(parseInt(ins.attr("scale")) > 0 ? parseFloat(ins.attr("scale")) : 1);
                        var val = parseFloat(ins.val()) / scale; console.log(val);
                        MAV.send_message(mavlink10.MAVLINK_MSG_ID_PARAM_SET, {param_id: 'FILTER_KP_ACC', param_value: val, param_type: 9}, param2, 0, 1);
                    }    
                                        var param2 = function (){
                
                        var ins = $('input[name="PARAM_FILTER_KI"]');
                        var scale = parseFloat(parseInt(ins.attr("scale")) > 0 ? parseFloat(ins.attr("scale")) : 1);
                        var val = parseFloat(ins.val()) / scale;console.log(val);
                        MAV.send_message(mavlink10.MAVLINK_MSG_ID_PARAM_SET, {param_id: 'FILTER_KI', param_value: val, param_type: 9}, param3, 0, 1);
                    }    
                                        var param3 = function (){
                        
                        var ins = $('input[name="PARAM_FILTER_ACCEL_MARGIN"]');
                        var scale = parseFloat(parseInt(ins.attr("scale")) > 0 ? parseFloat(ins.attr("scale")) : 1);
                        var val = parseFloat(ins.val()) / scale;   console.log(val);                
                        MAV.send_message(mavlink10.MAVLINK_MSG_ID_PARAM_SET, {param_id: 'FILTER_ACCMARGIN', param_value: val, param_type: 9}, param4, 0, 1);
                    }    
                                        var param4 = function (){
                       
                        var ins = $('input[name="PARAM_GYRO_XY_ALPHA"]');
                        var scale = parseFloat(parseInt(ins.attr("scale")) > 0 ? parseFloat(ins.attr("scale")) : 1);
                        var val = parseFloat(ins.val()) / scale;  console.log(val);  
                        MAV.send_message(mavlink10.MAVLINK_MSG_ID_PARAM_SET, {param_id: 'GYROXY_LPF_ALPHA', param_value: val, param_type: 9}, param5, 0, 1);
                    }     
                                        var param5 = function (){
                      
                        var ins = $('input[name="PARAM_GYRO_Z_ALPHA"]');
                        var scale = parseFloat(parseInt(ins.attr("scale")) > 0 ? parseFloat(ins.attr("scale")) : 1);
                        var val = parseFloat(ins.val()) / scale;  console.log(val);  
                        MAV.send_message(mavlink10.MAVLINK_MSG_ID_PARAM_SET, {param_id: 'GYROZ_LPF_ALPHA', param_value: val, param_type: 9}, param6, 0, 1);
                    }    
                                        var param6 = function (){
            
                        var ins = $('input[name="PARAM_ACC_ALPHA"]');
                        var scale = parseFloat(parseInt(ins.attr("scale")) > 0 ? parseFloat(ins.attr("scale")) : 1);
                        var val = parseFloat(ins.val()) / scale;console.log(val);   
                        MAV.send_message(mavlink10.MAVLINK_MSG_ID_PARAM_SET, {param_id: 'ACC_LPF_ALPHA', param_value: val, param_type: 9}, param7, 0, 1);
                    }    
                                        var param7 = function (){ 
             
                        var ins = $('input[name="PARAM_GROUND_LEVEL"]');
                        var scale = parseFloat(parseInt(ins.attr("scale")) > 0 ? parseFloat(ins.attr("scale")) : 1);
                        var val = parseFloat(ins.val()) / scale;  console.log(val); 
                        MAV.send_message(mavlink10.MAVLINK_MSG_ID_PARAM_SET, {param_id: 'GROUND_LEVEL', param_value: val, param_type: 9}, param8, 0, 1);
                    }    
                                        var param8 = function (){ 
             
                        var ins = $('input[name="PARAM_FILTER_KP_EXT"]');
                        var scale = parseFloat(parseInt(ins.attr("scale")) > 0 ? parseFloat(ins.attr("scale")) : 1);
                        var val = parseFloat(ins.val()) / scale;  console.log(val); 
                        MAV.send_message(mavlink10.MAVLINK_MSG_ID_PARAM_SET, {param_id: 'FILTER_KP_EXT', param_value: val, param_type: 9}, next_callback, 0, 1);
                    }     
                    var val1 = parseInt($('input[name="PARAM_INIT_TIME"]').val());
                    MAV.send_message(mavlink10.MAVLINK_MSG_ID_PARAM_SET, {param_id: 'FILTER_INIT_T', param_value: val1, param_type: 6}, param1, 0, 1); 
            ...
            function save_mixer_config() {
                ...
                MAV.send_message(mavlink10.MAVLINK_MSG_ID_PARAM_SET, {param_id: 'MIXER', param_value: MIXER_CONFIG.mixer, param_type: 6}, next_callback, 0, 1);
            ...
            function save_board_alignment_config() {
                ...
                MAV.send_message(mavlink10.MAVLINK_MSG_ID_PARAM_SET, {param_id: 'FC_ROLL', param_value: BOARD_ALIGNMENT_CONFIG.roll, param_type: 9}, ()=>{
                    MAV.send_message(mavlink10.MAVLINK_MSG_ID_PARAM_SET, {param_id: 'FC_PITCH', param_value: BOARD_ALIGNMENT_CONFIG.pitch, param_type: 9}, ()=>{
                        MAV.send_message(mavlink10.MAVLINK_MSG_ID_PARAM_SET, {param_id: 'FC_YAW', param_value: BOARD_ALIGNMENT_CONFIG.yaw, param_type: 9},  next_callback, 0, 1);
                    }, 0, 1);                 
                }, 0, 1); 
            ...
            function save_motor_config() {
                ...
                if(false) {
                    ...
                    MAV.send_message(mavlink10.MAVLINK_MSG_ID_PARAM_SET, {param_id: 'MOTOR_IDLE_THR', param_value: (MOTOR_CONFIG.mincommand/2000), param_type: 9}, next_callback, 0, 1);
                }
            }
            ...
            function save_rc_deadband() {
                ...
                //next_callback();
                MAV.send_message(mavlink10.MAVLINK_MSG_ID_PARAM_SET, {param_id: 'RC_TYPE', param_value: 0, param_type: 6}, 0, next_callback, 1);
            ...
            function save_to_eeprom() {
                rosflightIO.save_parameter(reboot);
                ;
