...
TABS.pid_tuning.initialize = function (callback) {
    ...
    function update_pid_val(contr){
        PID.controller = (contr!=undefined) ? contr : rosflightIO.handle_get_param("RC_ATT_MODE");
        PID_names = ["ROLL","PITCH","YAW"];
        var pnames = ["P","I","D"];
        
        PID_names.forEach(function(pidname, index) {
            var mode = (PID.controller==1) ? "ANG" : "RATE"; 
            PIDs[index] = [];
            pnames.forEach(function(pidtype, z) {
                if(mode=="ANG" && pidname=="YAW"){
                    return;
                }
                var param_name = "PID_"+pidname+"_"+mode+"_"+pidtype;
                console.log(param_name, "mode "+mode);
                var val = rosflightIO.handle_get_param(param_name);
                PIDs[index].push(val);
            });
            
        });

        RC_tuning.roll_rate = (rosflightIO.handle_get_param("RC_MAX_ROLLRATE"));
        RC_tuning.pitch_rate = (rosflightIO.handle_get_param("RC_MAX_PITCHRATE"));
        RC_tuning.yaw_rate = (rosflightIO.handle_get_param("RC_MAX_YAWRATE"));
        
        RC_tuning.RC_RATE = (rosflightIO.handle_get_param("RC_MAX_ROLL"));
        RC_tuning.RC_EXPO = (rosflightIO.handle_get_param("ARM_THRESHOLD"));

        RC_tuning.dynamic_THR_PID = (rosflightIO.handle_get_param("PID_TAU"));
        
        
        $('.pid_tuning input[name="rc_rate"]').attr("max", Math.PI);
        $('.pid_tuning input[name="roll_rate"]').attr("max", Math.PI*3);
        $('.pid_tuning input[name="pitch_rate"]').attr("max", Math.PI);
        $('.pid_tuning input[name="yaw_rate"]').attr("max", Math.PI);
    }

    TABS.pid_tuning.update_pid_val = update_pid_val;
    ...
    TABS.pid_tuning.pid_and_rc_to_form = pid_and_rc_to_form;
    ...
    function process_html() {
        ...
        if (semver.lt(CONFIG.apiVersion, "1.31.0")) {
            pidController_e.change(function () {
                ...
                self.update_pid_val(pidController_e.val());
                self.pid_and_rc_to_form();
                self.updatePidControllerParameters();
            });
        } 
