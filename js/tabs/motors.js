...
TABS.motors.initialize = function (callback) {
    ...
    function update_model(mixer) {
        ...
        mixer = mixer>mixerList.length ? 0 : mixer;
        //if (semver.gte(CONFIG.apiVersion, "1.36.0")) {
    ...
    function process_html() {
        ...
        $('.tab-motors .rate select, .tab-motors .scale select').change(function () {
            ...
            case "gyro":
                ...
                GUI.interval_add('IMU_pull', function imu_data_pull() {
                    ...
                    SENSOR_DATA.gyroscope[0] = rosflightIO.imu_msg.gyro.x;
                    SENSOR_DATA.gyroscope[1] = rosflightIO.imu_msg.gyro.y;
                    SENSOR_DATA.gyroscope[2] = rosflightIO.imu_msg.gyro.z;
                    update_gyro_graph();
            ...
            case "accel":
                ...
                GUI.interval_add('IMU_pull', function imu_data_pull() {
                      SENSOR_DATA.accelerometer[0] = rosflightIO.imu_msg.acc.x;
                      SENSOR_DATA.accelerometer[1] = rosflightIO.imu_msg.acc.y;
                      SENSOR_DATA.accelerometer[2] = rosflightIO.imu_msg.acc.z;
                      update_accel_graph();
        ...
        if (self.escProtocolIsDshot) {
            ...
            rangeMin = parseFloat(MOTOR_CONFIG.mincommand.toFixed(3));
        ...
        .prop('max', rangeMax);
        $('div.values li:not(:last)').text(rangeMin.toFixed(3));
        ...
        function mmap( x,  in_min,  in_max,  out_min,  out_max) {
        ...
        $('#motorsEnableTestMode').change(function () {
            ...
            rosflightIO.setArmingEnabled(enabled, enabled);
        }).change();
        ...
        $('div.sliders input:not(.master)').on('input', function () {
            ...
            var cmd_array = [];
            var val = $(this).val();
            $('div.values li').eq(index).text(val);

            for (var i = 0; i < 8; i++) {
                var val = parseInt($('div.sliders input').eq(i).val());
                cmd_array.push(mmap(val, 0, 2000, 0.0, 1.0)); 
                buffer.push16(val);
            }

            buffering_set_motor.push(buffer);
            //2 motor, 1 servo
            var buf_data = {type_array:[2,2,2,2,1,1,1,1,1,1,1,1], aux_cmd_array:cmd_array};
...
//console.log(buf_data);
                    ...
                    MAV.send_message(mavlink10.MAVLINK_MSG_ID_ROSFLIGHT_AUX_CMD, buf_data, false, false, true);
        ...
        function get_motor_data() {
            ...
            for(var i=0;i<rosflightIO.out_msg.rc.values.length;i++)
                MOTOR_DATA.push16(rosflightIO.out_msg.rc.values[i]);
        }
